...
const sign = (privKey:string, plain: any, set: Function) => {
  let rsa = new NodeRSA();
  rsa.importKey(privKey, "private");
  set(rsa.sign(plain, "base64", "utf8"));
};

export default function RsaSign() {
  const [ privKey, setPrivKey ] = useState("");
  const [ plain, setPlain ] = useState("");
  const [ result, setResult ] = useState("");
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px'}}>
      <Title level={2} style={{ marginTop: 0 }}>RSA Sign</Title>
      <div style={{ display: 'flex', gap: 16, alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', alignItems:'center', height: 330 }}>
            <Button type="primary" onClick={ () => sign(privKey, plain, setResult) }>Sign</Button>
          </div>
        </div>
        <div style={{ flexGrow: 1}}>
